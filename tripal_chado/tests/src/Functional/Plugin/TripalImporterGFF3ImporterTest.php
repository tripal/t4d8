...
class TripalImporterGFF3ImporterTest extends ChadoTestBrowserBase {
  ...
  public function testGFFImporterSimpleTest() {
    ...
    // This uses the schemaName variable within this class
    ...
    // Create dbxref for organism
    $dbxref_values = array(
      'db_id' => 1,
      'accession' => 'organism'
    );
    $dbxref_results = $connection->query('SELECT * FROM ' . $chado_name . '.dbxref WHERE accession=:a', [':a' => 'organism'])->fetchObject();
    if($dbxref_results == NULL) {
      $dbq = chado_insert_record('dbxref', $dbxref_values, [], $chado_name);
    }
    

    // Create analysis
    $analysis_values = array(
      'name' => 'Test Analysis',
      'description' => 'Test Analysis',
      'program' => 'PROGRAM',
      'programversion' => '1.0',
    );
    $analysis = $connection->query('SELECT * FROM ' . $chado_name . '.analysis WHERE name=:n', [':n' => 'Test Analysis'])->fetchObject();
    if($analysis == NULL) {
      $dbq = chado_insert_record('analysis', $analysis_values, [], $chado_name);
      $analysis = $connection->query('SELECT * FROM ' + $chado_name + '.analysis WHERE name=:n', [':n' => 'Test Analysis'])->fetchObject(); 
    }

    // Add a sample organism
    $org_values = [
      'genus' => 'Tripalus',
    ...
    $organism = $connection->query('SELECT * FROM ' . $chado_name . '.organism
    WHERE genus=:g AND species ILIKE :s',
    [':g' => $org_values['genus'], ':s' => 'ferox%'])->fetchObject();
    if($organism == NULL) {
      $dbq = chado_insert_record('organism', $org_values, [], $chado_name);

      // Get organism data from the table
      $organism = $connection->query('SELECT * FROM ' . $chado_name . '.organism
        WHERE genus=:g AND species=:s',
        [':g' => $org_values['genus'], ':s' => $org_values['species']])->fetchObject();
    }
    // echo "\n\n\n\nOrganism:";
    // print_r($organism);
    // echo "\n\n\n\n";
    ...
    // TODO: This code still performs changes to chado (schemaName cannot be overriden yet: TO FIX)

    $gff3_importer = new GFF3Importer();
    // $gff3_importer->schema_name = 'testchado';
    // $gff_file = ['file_local' => __DIR__ . '/../data/small_gene.gff']; // T3 code
    $gff_file = __DIR__ . '/../data/small_gene.gff';
    $fasta = ['file_local' => __DIR__ . '/../data/short_scaffold.fasta'];
    // $analysis = factory('chado.analysis')->create(); // T3 code
    // $organism = factory('chado.organism')->create(); // T3 code
    $run_args = [
    ...
    // OLD T3 CODE
    // $gff3_importer->runGFFLoader($run_args, $gff_file);
    // NEW T4 CODE
    $gff3_importer->run();
  ...
  private function loadLandmarks($analysis, $organism, $landmark_file = array()) {
    ...
    $importer->schema_name = 'testchado';
